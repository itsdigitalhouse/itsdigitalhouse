...
import WelcomePopup from './WelcomePopup'; // WelcomePopup component dynamically linked
...
const Navbar = () => {
  ...
  // Custom State for Get A Consultation Modal Popup
  ...
  return (
    <>
      ...
      isHeaderWhite
        ...
        <div className="max-w-screen-2xl mx-auto px-6 lg:px-12 h-full">
          <div className="flex items-center justify-between h-full">
            ...
            {/* Desktop Consultation Button - Controls Modal Toggle */}
            <button 
              onClick={() => setIsConsultationOpen(true)}
              className="hidden lg:block font-black uppercase text-[11px] border-2 border-slate-950 rounded-full px-6 py-2 hover:bg-slate-950 hover:text-white transition-all transform-gpu active:scale-95"
            >
              Get A Consultation
            </button>
            ...
            <div className="lg:hidden flex items-center">
              <button 
                ...
                className="text-slate-900 focus:outline-none p-2 relative z-[2001]"
              >
                {isMobileMenuOpen ? (
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div 
              ...
              className="fixed inset-0 top-0 left-0 w-full h-screen h-[100vh] min-h-screen z-[2000] bg-white flex flex-col lg:hidden"
            >
              {/* Header inside Menu */}
              <div className="flex justify-between items-center p-6 border-b border-slate-100 h-24 min-h-[96px]">
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-white">
                <div className="flex flex-col gap-4 pb-6">
                  {navLinks.map((link) => (
                    <div key={link.name} className="border-b border-slate-50 pb-3">
                      {!link.hasDropdown ? (
                        <Link to={link.path} onClick={() => setIsMobileMenuOpen(false)} className="font-bold uppercase tracking-wider text-[14px] text-slate-700 block py-2">
                          {link.name}
                        </Link>
                      ) : (
                        <div>
                          <button 
                            ...
                            className="w-full flex justify-between items-center font-bold uppercase tracking-wider text-[14px] text-slate-700 py-2"
                          >
                            {link.name}
                              ...
                              >
                                {link.name === 'Services' ? (
                                    ...
                                    {Object.values(servicesData).map(s => (
                                      <div key={s.title}>
                                        ...
                                        <div className="space-y-2 pl-2">
                                          {s.items.map(i => (
                                            <Link key={i} to={`/services/${i.toLowerCase().replace(/\s+/g, '-')}`} onClick={() => setIsMobileMenuOpen(false)} className="block text-[12px] font-medium text-slate-600 py-1">• {i}</Link>
                                          ))}
                                        </div>
                                      </div>
                                    ))}
                                  ...
                                  <div className="pl-2 mt-2 flex flex-col gap-1 bg-slate-50 p-3 rounded-xl">
                                    {solutionsData.map(item => (
                                      <Link key={item.name} to={item.path} onClick={() => setIsMobileMenuOpen(false)} className="text-[12px] font-semibold text-slate-600 p-2 block hover:text-[#ee3444] transition-colors">{item.name}</Link>
                                    ))}
                                  </div>
                                )}
                              </motion.div>
                ...
                {/* Mobile Consultation Button Inside Sidebar */}
                <div className="mt-4 pb-8">
                  <button 
                    ...
                    className="w-full font-black uppercase text-[12px] text-center border-2 border-slate-950 rounded-full py-3 hover:bg-slate-950 hover:text-white transition-all transform-gpu"
                  >
                    Get A Consultation
                  </button>
                </div>
      ...
      {/* WelcomePopup Modal Trigger injection */}
      <WelcomePopup 
